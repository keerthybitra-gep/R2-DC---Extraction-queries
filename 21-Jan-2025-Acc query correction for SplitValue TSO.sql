...
WHERE doc.IsDeleted = 0
      AND EntityDetails.LOBEntityDetailCode in (select entityDetailCode from @LOBIDs)  /*commented for testing */
	  ...
	  and doc.DocumentNumber in  ('0060549531-002')  --('0060012030-001','0060012110-001','0060012129','0060017964-001') 
	  --( '0060017964-001','0060565230-001','0060605574','0060008715-002')   --('0060018132-008')  --('0060008715-002','0060017964-001','0061066480')  --(','0061066480','0061117428','0061118500','0061123528','0061123592','0061129182');
	  ;
...
WHERE POI.ItemExtendedType = 1
      AND POI.ClosingOrderStatus <> 124
      --AND POI.IsCloseForReceiving = 0 /*TSO : commented on 01/23  for order#0061020043*/
      AND POI.IsDeleted = 0
      AND POI.ItemStatus <> 121
      AND D.IsDeleted = 0;
...
WHERE TML.DifferenceQuantity > 0;
...
(SELECT DISTINCT OrderID FROM #TempMaterialLinePOs) AS po
    ...
    INNER JOIN #LineTotalValueOfMaterialPOs_ALL AS LTP WITH (NOLOCK)
        ON TML.P2PLineItemId = LTP.P2PLineItemId
           AND LTP.OrderId = TML.OrderID
WHERE TML.DifferenceQuantity > 0; /*TSO : updated on 01-22 */
...
(
    ...
    CurrentOrderLineTotal DECIMAL(18, 6),
    OrderItemId BIGINT
	,SUnitPrice DECIMAL(18, 6) /*added on 01/21 */
);
...
SELECT DISTINCT
	...
	ISNULL(FML.Quantity, FSL.Quantity) AS Quantity, /* TSO: updated on  01/21 */
...
WHERE DM.DocumentStatus = 1 /* commented for testing */
      AND DM.IsDeleted = 0
...
----------------------------------- Order Ids Ended -----------------------------------------------
-------------------------------- PO Accounting Migration Data Started ----------------------------------
DECLARE @Precisionvalue INT = 3;

WITH contacts
AS (SELECT u.UserName AS UserName,
           upm.ContactCode AS ContactCode,
           u.FirstName + ' ' + u.LastName AS [Name]
    FROM UM_UserPartnerMapping AS upm WITH (NOLOCK)
        INNER JOIN um_users AS u WITH (NOLOCK)
            ON u.UserId = upm.UserId
   ),
     lle_details
AS (SELECT d.DocumentNumber AS [OrderNumber],
           items.OrderId AS [documentcode],
           items.OrderItemID AS [DocumentItemID],
           items.LineNumber AS [OrderLineNumber],
           items.P2PLineItemID AS P2PLineItemID,
           splitfieldconfig.EntityTypeId AS [EntityId],
           splitentities.EntityCode AS [EntityDetailCode],
           splitfieldconfig.FieldName AS [FieldName],
           CASE
               WHEN splitfieldconfig.FieldControlType = 4 THEN
                   CAST(splitentities.OrderSplitItemEntityId AS NVARCHAR(200))
               ELSE
                   splitentities.SplitAccountingFieldValue
           END AS [FieldValue],
           so.OrderSplitItemId AS [DocumentSplitItemId],
           so.SplitType AS SplitType,
           CASE
               WHEN SO.SplitType = 0
                    AND items.ItemExtendedType = 1 THEN
                   ROUND((TML.DifferenceQuantity / Items.Quantity) * SO.Quantity, 4) -- Split by QTY	
               WHEN SO.SplitType = 0
                    AND items.ItemExtendedType != 1 THEN
           (TSL.DifferenceAmount / NULLIF(Items.UnitPrice, 0)) * SO.Quantity         -- Split by amount
               WHEN SO.SplitType = 1 THEN
                   SO.Percentage                                                     -- Split by percentage
           END AS Quantity,
           so.Percentage AS Percentage,
           CASE
               WHEN SO.SplitType = 0
                    AND items.ItemExtendedType = 1 THEN
                   ROUND(
                            (((TML.DifferenceQuantity / Items.Quantity) * SO.Quantity) * items.UnitPrice)
                            + (((TML.DifferenceQuantity / Items.Quantity) * SO.Quantity) * so.Tax),
                            @Precisionvalue
                        ) -- Split by QTY	
               WHEN SO.SplitType = 0
                    AND items.ItemExtendedType != 1 THEN
                   ROUND(
                            (((TSL.DifferenceAmount / NULLIF(Items.UnitPrice, 0)) * SO.Quantity)
                             + ((TSL.DifferenceAmount / NULLIF(Items.UnitPrice, 0)) * SO.Tax)
										),
										@Precisionvalue
									) -- Split by amount
						   WHEN SO.SplitType = 1
								AND items.ItemExtendedType = 1 THEN
							   ROUND(
										((SO.Percentage * TML.DifferenceQuantity * items.UnitPrice) / 100)
										+ ((TML.DifferenceQuantity / items.Quantity) * SO.Tax),
										@Precisionvalue
									)
						   WHEN SO.SplitType = 1
								AND items.ItemExtendedType != 1 THEN
							   ROUND(
										((SO.Percentage * TSL.DifferenceAmount) / 100)
										+ ((TSL.DifferenceAmount / items.UnitPrice) * SO.Tax),
										@Precisionvalue
									)
					   END AS [SplitItemTotal],
			--so.SplitItemTotal as [SplitItemTotal], /*TSO: commented  on 01-22-2025*/
           ...
           (TSL.DifferenceAmount / items.UnitPrice) * SO.Tax
		       WHEN SO.SplitType = 0
                    AND items.ItemExtendedType = 1 THEN
           ((TML.DifferenceQuantity / Items.Quantity)* SO.Quantity) * SO.Tax /*TSO: updated  on 01-25-2025*/
               WHEN SO.SplitType = 0
                    AND items.ItemExtendedType != 1 THEN
           (TSL.DifferenceAmount / NULLIF(Items.UnitPrice, 0)) * SO.Tax
           END AS TAX, 
		   --SO.Tax AS TAX, /* commented  on 01-22-2025*/
           so.ShippingCharges AS ShippingCharges,
           so.AdditionalCharges AS AdditionalCharges,
           so.OrderSplitItemId AS OrderSplitItemId,
           items.ItemTypeID AS ItemTypeID,
           so.SplitLineNumber AS [Split Number]
    FROM dbo.P2P_OrderSplitItemEntities AS splitentities WITH (NOLOCK)
        INNER JOIN dbo.P2P_OrderSplitItems AS so WITH (NOLOCK)
            ON so.OrderSplitItemId = splitentities.OrderSplitItemId
               AND so.IsDeleted = 0
        INNER JOIN dbo.P2P_OrderItems AS items WITH (NOLOCK)
            ON items.OrderItemID = so.OrderItemId
               AND items.IsDeleted = 0
        INNER JOIN P2P_Orders AS O WITH (NOLOCK)
            ON O.OrderID = items.OrderId
        INNER JOIN #OrderIds AS OrderIds WITH (NOLOCK)
            ON OrderIds.DocumentCode = O.OrderID
        INNER JOIN #FinalOrderList AS PO_List WITH (NOLOCK)
            ON PO_List.Orderid = O.OrderID
               AND PO_List.Orderitemid = items.OrderItemID
        INNER JOIN dbo.P2P_SplitAccountingFieldConfigurations AS splitfieldconfig WITH (NOLOCK)
            ON splitfieldconfig.SplitAccountingFieldConfigId = splitentities.SplitAccountingFieldConfigId
               AND splitfieldconfig.DocumentType = 8
               AND splitfieldconfig.LevelType = 2
        INNER JOIN dm_documents AS d WITH (NOLOCK)
            ON d.DocumentCode = items.OrderId
               AND d.IsDeleted = 0
               AND d.DocumentTypeCode = 8
        INNER JOIN P2P_OrderEntityDetails AS ORE WITH (NOLOCK)
            ON ORE.OrderID = O.OrderID
        INNER JOIN DM_DocumentBU AS BU WITH (NOLOCK)
            ON BU.DocumentCode = D.DocumentCode
        INNER JOIN ORG_EntityDetails AS DBU WITH (NOLOCK)
            ON BU.BUCode = DBU.EntityDetailCode
               AND DBU.EntityId = 9
        INNER JOIN DM_DocumentLOBMapping AS lob WITH (NOLOCK)
            ON lob.DocumentCode = d.DocumentCode
        INNER JOIN ORG_EntityDetails AS OE WITH (NOLOCK)
            ON OE.EntityDetailCode = lob.EntityDetailCode
        INNER JOIN @Status AS sts
            ON sts.StatusId = D.DocumentStatus
        ...
        LEFT JOIN #LegacyOrderAmount AS LOA WITH (NOLOCK)
            ON LOA.OrderId = O.OrderID
               AND LOA.P2PLineitemId = items.P2PLineItemID
        LEFT JOIN #TempMaterialLinePOs AS TML WITH (NOLOCK)
            ON TML.P2PLineItemId = items.P2PlineItemId
               AND TML.OrderID = items.OrderId
        LEFT JOIN #TempServiceLinePOs AS TSL WITH (NOLOCK)
            ON TSL.P2PLineItemId = items.P2PlineItemId
               AND TSL.OrderID = items.OrderId
    WHERE so.IsDeleted = 0 /*commented for testing */
          AND (
...
SELECT DISTINCT
    OrderNumber AS [OrderNumber (UniqueKey)*],
    OrderLineNumber AS [OrderLineNumber*],
    OrderSplitItemId AS [OrderSplitItemId],
    [Split Number] AS [Split Number*],
   CASE
        WHEN SplitType = 0
             AND ItemTypeID = 1 THEN
            'Quantity'
        WHEN SplitType = 0
             AND ItemTypeID > 1 THEN
            'Amount'
        WHEN SplitType = 1 THEN
            'Percentage'
        ELSE
            'None'
    END AS [SplitType*], 
	--CASE    
	--		WHEN base.SplitType !=0 THEN 'Percentage'    
	--		WHEN base.SplitType = 0 and base.ItemTypeID  in (1)  THEN 'Quantity' 
	--		WHEN base.SplitType = 0 and base.ItemTypeID  !=1  THEN 'Amount' 
	--		WHEN base.SplitType = 2 THEN 'None'
	--	  END AS [SplitType], 
    /* CASE SplitType
        WHEN 0 THEN
            ISNULL(Quantity, 0)
    ...
    END AS [SplitValue*], */
		...
		CASE    
			WHEN base.SplitType !=0 THEN Percentage  
			WHEN base.SplitType = 0 and base.ItemTypeID  in(1) then base.Quantity
			WHEN base.SplitType = 0 and base.ItemTypeID  !=1 then base.SplitItemTotal - base.Tax 
		END AS [SplitValue] , 

    SplitItemTotal AS [SplitItemTotal*],
    Tax AS [Taxes*],
    AdditionalCharges AS [AdditionalCharges*],
    ShippingCharges AS [ShippingCharges*],
    MAX(Period) AS [Period],
    MAX(Requester) AS [Requester*],
    MAX(org2Code) AS AE2,   -- [Division]      
    MAX(org3Code) AS AE3,   -- [Plant]      
    MAX(org5Code) AS AE5,   -- [Company code]      
    MAX(org6Code) AS AE6,   -- [GL Account]      
    MAX(org9Code) AS AE9,   -- [Business Unit]      
    MAX(org12Code) AS AE12, -- [Cost Center]      
    MAX(org22Code) AS AE22, -- [Purchasing Org]      
    MAX(org23Code) AS AE23, -- [Internal Order]      
    MAX(org24Code) AS AE24, -- [Item Category]      
    MAX(org25Code) AS AE25, -- [Purchasing Group]      
    MAX(org26Code) AS AE26, -- [Storage Location]      
    MAX(org27Code) AS AE27, -- [WBS Element]      
    MAX(org28Code) AS AE28, -- [Account Assignment Category]
    MAX(org29Code) AS AE29,
    MAX(org30Code) AS AE30,
    MAX(org31Code) AS AE31,
    MAX(org32Code) AS AE32,
    MAX(org33Code) AS AE33,
    MAX(org34Code) AS AE34,
    MAX(org35Code) AS AE35,
    MAX(org39Code) AS AE39,
    MAX(org40Code) AS AE40,
    MAX(org42Code) AS AE42, -- [WBS-TECO]      
    MAX(org43Code) AS AE43, -- [IO-TECO]      
    MAX(org44Code) AS AE44  -- [BU-Company Code]      
FROM
(
    SELECT DISTINCT
        lle.OrderSplitItemId,
        ItemTypeID,
        lle.[Split Number],
        lle.documentcode,
        lle.OrderNumber,
        ...
        lle.SplitItemTotal,
        lle.Tax,
        lle.AdditionalCharges,
        lle.ShippingCharges,
        CASE
            WHEN lle.EntityId = 0
                 AND lle.FieldName = 'Requester' THEN
                c.UserName
            ELSE
                ''
        END AS [Requester],
        CASE
            WHEN lle.EntityId = 0
                 AND lle.FieldName = 'Period' THEN
                lle.EntityDetailCode
            ELSE
                ''
        END AS [Period],
        CASE
            WHEN oed.EntityId = 1 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org1Code],
        CASE
            WHEN oed.EntityId = 2 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org2Code],
        CASE
            WHEN oed.EntityId = 3 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org3Code],
        CASE
            WHEN oed.EntityId = 4 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org4Code],
        CASE
            WHEN oed.EntityId = 5 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org5Code],
        CASE
            WHEN oed.EntityId = 6 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org6Code],
        CASE
            WHEN oed.EntityId = 7 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org7Code],
        CASE
            WHEN oed.EntityId = 8 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org8Code],
        CASE
            WHEN oed.EntityId = 9 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org9Code],
        CASE
            WHEN oed.EntityId = 10 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org10Code],
        CASE
            WHEN oed.EntityId = 11 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org11Code],
        CASE
            WHEN oed.EntityId = 12 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org12Code],
        CASE
            WHEN oed.EntityId = 13 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org13Code],
        CASE
            WHEN oed.EntityId = 14 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org14Code],
        CASE
            WHEN oed.EntityId = 15 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org15Code],
        CASE
            WHEN oed.EntityId = 16 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org16Code],
        CASE
            WHEN oed.EntityId = 17 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org17Code],
        CASE
            WHEN oed.EntityId = 18 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org18Code],
        CASE
            WHEN oed.EntityId = 19 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org19Code],
        CASE
            WHEN oed.EntityId = 20 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org20Code],
        CASE
            WHEN oed.EntityId = 21 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org21Code],
        CASE
            WHEN oed.EntityId = 22 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org22Code],
        CASE
            WHEN oed.EntityId = 23 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org23Code],
        CASE
            WHEN oed.EntityId = 24 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org24Code],
        CASE
            WHEN oed.EntityId = 25 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org25Code],
        CASE
            WHEN oed.EntityId = 26 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org26Code],
        CASE
            WHEN oed.EntityId = 27 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org27Code],
        CASE
            WHEN oed.EntityId = 28 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org28Code],
        CASE
            WHEN oed.EntityId = 29 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org29Code],
        CASE
            WHEN oed.EntityId = 30 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org30Code],
        CASE
            WHEN oed.EntityId = 31 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org31Code],
        CASE
            WHEN oed.EntityId = 32 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org32Code],
        CASE
            WHEN oed.EntityId = 33 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org33Code],
        CASE
            WHEN oed.EntityId = 34 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org34Code],
        CASE
            WHEN oed.EntityId = 35 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org35Code],
        CASE
            WHEN oed.EntityId = 36 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org36Code],
        CASE
            WHEN oed.EntityId = 37 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org37Code],
        CASE
            WHEN oed.EntityId = 38 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org38Code],
        CASE
            WHEN oed.EntityId = 39 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org39Code],
        CASE
            WHEN oed.EntityId = 40 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org40Code],
        CASE
            WHEN oed.EntityId = 41 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org41Code],
        CASE
            WHEN oed.EntityId = 42 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org42Code],
        CASE
            WHEN oed.EntityId = 43 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org43Code],
        CASE
            WHEN oed.EntityId = 44 THEN
                oed.EntityCode
            ELSE
                ''
        END AS [org44Code],
        'CVX1' AS COA
    FROM lle_details AS lle
        LEFT JOIN contacts AS c
            ON c.ContactCode = lle.FieldValue
        LEFT JOIN dbo.ORG_EntityDetails AS oed WITH (NOLOCK)
            ON lle.EntityId = oed.EntityId
               AND lle.EntityDetailCode = oed.EntityCode
               AND (
                       (
                           ISNULL(oed.EntityId, 0) = 0
                           AND ISNULL(oed.EntityDetailCode, 0) = 0
                       )
                       OR (
                              ISNULL(oed.EntityId, 0) <> 0
                              AND ISNULL(oed.EntityDetailCode, 0) <> 0
...
GROUP BY OrderSplitItemId,
         ...
         ItemTypeID,
         base.[Split Number]
ORDER BY base.OrderNumber,
         base.OrderLineNumber ASC;
